            ...
            >
                <div className="institutionCheck-form">
                    <Form layout="vertical">
                        ...
                        {/*<FormItem className="companyName" {...formItemLayout_14} label="所属分公司：">
                            {getFieldDecorator('parentName', {
                                ...
                                <Input disabled placeholder="请输入所属分公司"/>
                            )}
                        </FormItem>*/}
...
class DataTable extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            pagination: {
                current: 1,
                pageSize: Number(localStorage.institutionCheckPageSize) || 10,
                ...
                if (json.result === 0) {
                    ...
                    json.data.list.forEach((item, index) => {
                        ...
                        data.push({
                            ...
                            index: index + 1,
                            institutionName: item.name || "暂无",
                            // areaName: item.areaName || "暂无",
                            areaName: item.provinceName + item.cityName + item.areaName + item.street,
                            // logo: item.icon ? global.config.photoUrl + item.icon : "",
                            // photo: item.photo ? global.config.photoUrl + item.photo : "",
                            // 没有logo图片，列表显示数据图片里的第一张
                            // photo: item.resourceList[0] ? item.resourceList[0].path : "",
                            photo: tempIcon,
                            name: item.name || "暂无",
                            parentName: item.parentName || "暂无",
                            parentId: item.parentId,
                            // typeName: item.typeName.split(",").join("/"),
                            typeName: item.typeName ? item.typeName.replace(/,/g, "/") : '',
                            // typeName: (item.typeName || "暂无") + (item.typeNameTwo ? ("/" + item.typeNameTwo) : "") + (item.typeNameThree ? ("/" + item.typeNameThree) : ""),
                            // telephone: item.telephone,
                            telephone: item.managerPhone || "暂无",
                            managerName: item.managerName || "暂无",
                            createTime: item.createTime ? this.dateHandle(item.createTime) : "暂无",
                            createUser: item.createUserName || "暂无",
