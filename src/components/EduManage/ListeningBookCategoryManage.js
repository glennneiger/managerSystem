...
class DataTable extends Component {
    ...
    dataHandle = (data) => {
        ...
        data.list.forEach((item, index) => {
            const temp = {
                key: item.listenBookType.id,
                id: item.listenBookType.id,
                parentId: item.listenBookType.parentId,
                index: index + 1,
                name: item.listenBookType.name,
                Icon: item.listenBookType.icon,
                sort: item.listenBookType.sort ? item.listenBookType.sort : '',
            };
            if (item.children) {
                const tempChildren = [];
                ...
                dataItem.forEach((subItem, subIndex) => {
                    const temp = {
                        key: subItem.id,
                        id: subItem.id,
                        sort: subItem.sort ? subItem.sort : '',
    ...
    handleSort = (row) => {
        ...
        reqwest({
            url: '/sys/listenBookType/updateSort',
            type: 'json',
            method: 'post',
            data: {
                // 机构Id
    ...
    itemDelete = (para) => {
        ...
        reqwest({
            url: '/sys/listenBookType/delete?id=' + para,
            type: 'json',
            method: 'delete',
            headers: {
                Authorization: sessionStorage.token
...
class ListeningBookCategoryManage extends Component {
    constructor(props) {
        super(props);
        this.state = {
            opObj: {},
...
export default ListeningBookCategoryManage;