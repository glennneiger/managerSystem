...
const ItemAddForm = Form.create()(
    (props) => {
        const {form, channels, optionsOfType, reqwestUploadToken, viewPic, effectPic, data_pic, setViewPic, picUpload, avatarEditor, setAvatarEditor, photoLoading, provinceList, cityCode} = props;
        const {getFieldDecorator} = form;

        //频道选项生成
        const optionsOfChannel = [];
        channels.forEach((item) => {
            optionsOfChannel.push({value: item.growthType.id, label: item.growthType.name});
        });
        ...
        return (
            <Form>
                <FormItem className="title" {...formItemLayout_14} label="成长标题：">
                    {getFieldDecorator('title', {
                        ...
                        <Input placeholder="请输入成长标题（6-42字）"/>
                    )}
                </FormItem>
                <FormItem className="typeIds" {...formItemLayout_8} label="成长分类：">
                    {getFieldDecorator('typeIds', {
                        ...
                        /*<Cascader options={optionsOfChannel} placeholder="请选择成长分类"/>*/
                    )}
                </FormItem>
                <FormItem className="author" {...formItemLayout_8} label="成长来源：">
                    {getFieldDecorator('author', {
                        ...
                        <Input placeholder="请填写成长来源（不超过10个字）"/>
                    )}
                </FormItem>
                {/*<FormItem
                ...
                <FormItem className="cityId" {...formItemLayout_8} label="展示城市：">
                ...
                <FormItem className="summary" {...formItemLayout_16} label="成长概述：">
                    {getFieldDecorator('summary', {
                        ...
                        <TextArea style={{resize: "none"}} placeholder="请填写成长概述" rows={10}/>
                    )}
                </FormItem>
            </Form>
        )
...
class ItemAdd extends Component {
    ...
    handleCreate = () => {
        ...
        form.validateFields((err, values) => {
            ...
            reqwest({
                ...
                data: {
                    title: values.title,
                    type: values.typeIds[1] || values.typeIds[0],
    ...
    componentDidMount() {
        ...
        console.log(this.props.optionsOfType)
    }

    render() {
        return (
            <div className="add-information" style={{minHeight: 725, display: this.props.status ? "block" : "none"}}>
                <header>
...
const ItemEditForm = Form.create()(
    (props) => {
        const {form, data, channels, typeList, optionsOfType, reqwestUploadToken, viewPic, effectPic, data_pic, setViewPic, picUpload, avatarEditor, setAvatarEditor, photoLoading, provinceList} = props;
        const {getFieldDecorator} = form;
        ...
        // 成长分类选项生成
        const optionsOfType02 = [{value: "0", label: "全国"}];
        let currentType02 = [];
            ...
            if (item.children) {
                item.children.forEach((subItem) => {
                    ...
                    if (Number(subItem.id) === data.typeId) {
                        // 当前城市设为选中项
                        currentType02 = [item.growthType.id, subItem.id]
                    }
                });
            }
            optionsOfType02.push({value: item.growthType.id, label: item.growthType.name, children: children});
        });
        ...
        return (
            <Form>
                ...
                <FormItem className="typeIds" {...formItemLayout_8} label="成长分类：">
                    {getFieldDecorator('typeIds', {
                        initialValue: data.typeId === 0 ? [""] : currentType02,
                        rules: [{
                            required: true,
                            message: '分类不能为空',
                        }],
                        ...
                        <Cascader options={optionsOfType} placeholder="请选择成长分类"/>
                    )}
                </FormItem>
                <FormItem className="author" {...formItemLayout_8} label="成长来源：">
                    {getFieldDecorator('author', {
...
class ItemEdit extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.status) {
            ...
            const rt= nextProps.record.cityId.toString();
        ...
        if (this.state.data) {
            dataSource = [
                ...
                <div className="description">
                    <span className="item-name">成长概述：</span>
                    <pre>
                        <span className="item-content">{this.state.data.summary}</span>
                    </pre>
                </div>,
                <div className="telephone">
                    {/*<span className="item-name">成长详情：</span>*/}
...
class DataTable extends Component {
    ...
    itemDelete = (para) => {
        ...
        reqwest({
            url: '/sys/growthNews/delete?id=' + para,
            type: 'json',
            method: 'delete',
            headers: {
                Authorization: sessionStorage.token
...
class GrowthManage extends Component {
    ...
    // 类型关键词设置
    setType = (value) => {
        // value = value === "全部分类" ? "" : value;
        this.setState({
            keyword: {
                newsName: this.state.keyword.newsName,
                type: value[1] || value[0],
                cityId: this.state.keyword.cityId,
            }
        })
    };
    ...
    setCity = (value) => {
        ...
        this.setState({
            keyword: {
                // ...this.state.keyword,
                type: this.state.keyword.setNewsName,                
            ...
            success: (json) => {
                if (json.result === 0) {
                    this.setState({
                        ...
                        //     tempArr.push(<Option key={index + 1} value={item.growthType.id}>{item.growthType.name}</Option>)
                        // });
                        // this.setState({
    ...
    typeList02 = () => {
        ...
        this.state.typeList.forEach((item) => {
            ...
            this.optionsOfType.push({value: item.growthType.id, label: item.growthType.name, children: children});
        });
    };
                        ...
                        <div>
                            <header className="clearfix">
                                ...
                                <Button 
                                    ...
                                    onClick={() => this.setAdd("status", true)}>添加成长</Button>
                            </header>
                            {/*成长列表*/}
                            ...
                            <ItemAdd 
                                ...
                                // optionsOfType={this.state.optionsOfType}
                                optionsOfType={this.optionsOfType}
