...
import TeachersOne from './TeachersOne';
import {
    Table,
    Input,
    InputNumber,
...
const ItemAddForm = Form.create()(
    (props) => {
        const {visible, onCancel, onCreate, form, allTypeList, typeList, checkPhone, checkTel,
            // checkOrgType, 
            handleChange, handleSearch, provinceList, cityList, districtList, streetList, markers, setMarkers, area, setArea, mapObj, setXY, setFormattedAddress, formattedAddress,
             // setPhone, countDown, codeButtonStatus, getCode, 
             reqwestUploadToken, viewPic01, setViewPic01, picUpload01, avatarEditor01, setAvatarEditor01, photoList01, setPhotoList01, photoLoading, viewPic02, effectPic02, data_pic02, setViewPic02, picUpload02, avatarEditor02, setAvatarEditor02, logoLoading, confirmLoading} = props;
...
class ItemAdd extends Component {
    ...
    checkTel = (rule, value, callback) => {
        ...
        if (!value) {
            ...
            const valuePhone = value.trim();
            if (isMob.test(valuePhone)||isPhone.test(valuePhone)) { // 正则验证
                callback(); // 校验通过
                return true;
            } else {
        ...
        reqwest({
            ...
            success: (json) => {
                if (json.result === 0) {
                    ...
                    // json.data.org.labels = JSON.parse(json.data.org.label);
                    // json.data.org.labels = json.data.org.label;
                    // console.log(JSON.parse(json.data.org.labels));
                    // json.data.orgTypeList.forEach((item, index) => {
                    //     data.typeIds.push({
                    //         typeId: item.typeId,
                ...
                <div className="institutions">
                    {
                        this.state.opObj.select ?
                            <div>
                                ...
                                {/*教师管理组件*/}
                                <Switch>
                                    <Route path="./CoursesOne" component={CoursesOne}></Route>
                                    <Route path="./TeachersOne" component={TeachersOne}></Route>
                                </Switch>
                            </div>
                            :
                            <p>暂无查询权限</p>
