                            ...
                            <Col span={8}>
                                <FormItem className="typeId" label="所属科目：">
                                    {getFieldDecorator('typeId', {
                                        ...
                                        <Cascader options={subjectOptions} onChange={onChange} placeholder="请选择所属教学科目"/>
                                    )}
                                </FormItem>
                            </Col>
...
class ItemAdd extends Component {
    ...
    copyCourse = (props1, event) => {
        // 判断条件有问题
        // if (sessionStorage.courseData) {
             // this.showModal(props1);
        // } else {
        //     this.countDown();
        // }
        this.getData();        
    ...
    // 获取所有的课程id, 根据id最大复制最近添加的一条记录
    getDataCoursesId = (keyword) => {
        this.setState({
            loading: true
        });
        reqwest({
            ...
            success: (json) => {
                ...
                if (json.result === 0) {
                    ...
                    json.data.list.forEach((item, index) => {
                        dataCoursesId.push(item.id);
                    });
                    courseId = Math.max.apply(null, dataCoursesId);
    ...
    handleCreate = (type) => {
        ...
        if (type === 2) {
            ...
            form.validateFieldsAndScroll((err, values) => {
                ...
                //         const tempParentId = item.parentId;
                //     }
                // })
...
const ItemEditForm = Form.create()(
    (props) => {
        ...
        //     let children = [];
        //     item.list.forEach((subItem) => {
        //         children.push({value: subItem.id, title: subItem.name})
        //     });
        //     subjectOptions.push({value: item.id, title: item.name, children: children});
        // });
                    ...
                    JSON.stringify(data) === "{}" ?
                        ...
                        <div className="course-edit course-form item-form">
                            <Form layout="vertical">
                                ...
                                {/*<Row gutter={24}>
                                    <Col>
                                        <FormItem className="tips longItem" label="购买须知：">
                                            {getFieldDecorator('tips', {
...
class ItemEdit extends Component {
    ...
    handleCancel = () => {
        ...
        // 课程特色写入-------------------------------------
        let characteristic = "";
        if (this.state.characteristicList) {
            this.state.characteristicList.forEach((item) => {
                if (!item.value) {
                    return;
                }
                    ...
                    return (
                        <div className="editable-row-operations">
                            ...
                            <Popconfirm title="确认下架?"
                                ...
                                <a style={{display: "inline"}}>下架</a>
                            </Popconfirm>
                            {/*课程推荐*/}                            
                            <Popconfirm title="确认推荐?"
                                        placement="topRight"
                                        onConfirm={() => this.itemBan(record.id)}
                                        onCancel=""
                                        okType="danger"
                                        okText="确认"
                                        cancelText="取消">
                                {/*<a style={{display: this.props.opObj.modify && record.statusCode === 2 ? "inline" : "none"}}>推荐</a>*/}
                                <a style={{display: "inline"}}>推荐</a>
                            </Popconfirm>
                        </div>
                    );
                },
