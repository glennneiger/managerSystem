...
class DataTableBasic extends Component {
    ...
    getData = () => {
        reqwest({
            ...
            method: 'get',
            data: {
                id: Number(this.props.id) || sessionStorage.orderDetailRecord.id,
            },
            headers: {
...
class DataTableStatistics extends Component {
    ...
    getData = () => {
        reqwest({
            url: '/sys/order/signList',
            type: 'json',
            method: 'get',
            data: {
                orderDetailsId: Number(this.props.orderDetailsId) || sessionStorage.orderDetailRecord.orderDetailsId,
            },
            headers: {
                Authorization: sessionStorage.token
            },
            ...
            success: (json) => {
                if (json.result === 0) {
                    ...
                    let consumerTime05 = '';
                    if (json.data.length) {
                        consumerTime01 = json.data[0] ? json.data[0].createTime : '';
                        consumerTime02 = json.data[1] ? json.data[1].createTime : '';
                        consumerTime03 = json.data[2] ? json.data[2].createTime : '';
                        consumerTime04 = json.data[3] ? json.data[3].createTime : '';
                        consumerTime05 = json.data[4] ? json.data[4].createTime : '';
                    }
                    data.push({
                    ...
                    this.setState({
                        data: data,
                        consumeNum: consumerTime01 === null ? 0 : json.data.length,
                    }, () => {
        ...
        return (
            <div>
                ...
                <Table bordered
                      ...
                      columns={this.columns}
                      footer={() => {
                        return <div>消费合计：<span style={{color: "red", fontWeight: "bold"}}>{this.state.consumeNum}/{(JSON.parse(sessionStorage.orderDetailRecord)).lessonNum}</span></div>
                      }}
                      />
            </div>
        )
...
class OrderDetail extends Component {
    ...
    componentWillMount() {
        ...
        if (this.props.location.search.substring(1) && this.props.location.search.substring(1) !== null) {
            // sessionStorage.orderDetailId = JSON.parse(this.props.location.search.substring(1));
            console.log(this.props.location.search.substring(1))
            sessionStorage.orderDetailRecord = this.props.location.search.substring(1);;
            console.log(sessionStorage.orderDetailRecord);
            console.log(JSON.parse(sessionStorage.orderDetailRecord));
        }       
        
        this.setState({
            id: (JSON.parse(sessionStorage.orderDetailRecord)).id,
            orderDetailsId: (JSON.parse(sessionStorage.orderDetailRecord)).orderDetailsId,
    ...
    // 返回按钮
    handleBack = () => {
        this.props.history.push('/index/order-manage/order');
